...
async function buscarExternoPorDocumento(documento: string): Promise<any | null> {
  const sanitize = (val: string | undefined, prefix: string): string => {
    if (!val) return ''
    let clean = val.trim()
    if (clean.startsWith(`${prefix}=`)) {
  ...
  if (!sisregUser || !sisregPassword) {
    console.warn('[SISREG Buscar Externo por Documento] Credenciais do SISREG não configuradas no servidor para consulta em tempo real.')
    return null
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!codigo && !documento) {
      return NextResponse.json({ ok: false, error: 'Parâmetro codigo ou documento é obrigatório.' }, { status: 400 })
    }

    let solicitacaoLocal: any = null
    if (codigo) {
      ...
      const { data } = await supabase
        .from('monitoramento_sisreg')
        .select('*')
        .or(`cpf_usuario.eq.${docLimpo},cns_usuario.eq.${docLimpo}`)
    ...
    if (!resultData && solicitacaoLocal) {
      console.log(`[SISREG Buscar GET] Busca externa falhou ou não retornou dados. Usando dados locais.`)
